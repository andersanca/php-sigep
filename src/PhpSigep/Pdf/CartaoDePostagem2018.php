...
class CartaoDePostagem2018
{
	...
	private function _render ($dest='', $fileName= '')
	{
		...
		while (count($objetosPostais)) {
            ...
            $lines = 4;
            // Observações
            $contato = substr($objetoPostal->getContato(),0,20);
            if (!empty($contato)) {
                $this->pdf->SetFontSize(9);
            ...
            $currentY = $this->pdf->GetY();
            $observacoes = substr($objetoPostal->getObservacao(),0,60);
	...
	{
		$this->pdf = new \PhpSigep\Pdf\ImprovedFPDF('P', 'mm', array(105, 140));
